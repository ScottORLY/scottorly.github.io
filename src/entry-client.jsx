...
const updateTime = () => {
    ...
    select(`#london-hand-hours`).transition().attr('transform', `rotate(${(london - timeDay(london)) / 864e5 * 720})`)
    select(`#london-hand-minutes`).transition().attr('transform', `rotate(${(london - timeHour(london)) / 36e5 * 360})`)
    select(`#london-hand-seconds`).transition().attr('transform', `rotate(${(london - timeMinute(london)) / 6e4 * 360})`)
